...
#include "cuda_phi_streebog_arrays.cuh"
...
__launch_bounds__(TPB, 3)
#endif
void phi_streebog_gpu_hash_64(uint64_t *g_hash){
...
__host__
void phi_streebog_cpu_hash_64(int thr_id, uint32_t threads, uint32_t *d_hash)
{
	dim3 grid((threads + TPB-1) / TPB);
	dim3 block(TPB);
	
	phi_streebog_gpu_hash_64 << <grid, block >> >((uint64_t*)d_hash);
}

__constant__ uint64_t target64[4];

void phi_streebog_set_target(const uint32_t* ptarget){
	cudaMemcpyToSymbol(target64,ptarget,4*sizeof(uint64_t),0,cudaMemcpyHostToDevice);
}

__global__
__launch_bounds__(TPB, 3)
void phi_streebog_gpu_hash_64_final(uint64_t *g_hash, uint32_t* resNonce)
{
	const uint32_t thread = (blockDim.x * blockIdx.x + threadIdx.x);
	uint2 buf[8], t[8], temp[8],K0[8], hash[8];
...
__host__
void phi_streebog_cpu_hash_64_final(int thr_id, uint32_t threads, uint32_t *d_hash, uint32_t* d_resNonce)
{
	dim3 grid((threads + TPB-1) / TPB);
	dim3 block(TPB);

	phi_streebog_gpu_hash_64_final << <grid, block >> >((uint64_t*)d_hash, d_resNonce);
}
